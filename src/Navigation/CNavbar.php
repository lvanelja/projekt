    ...
    public function create()
    {
        ...
        $createMenu = function ($items, $callback) use (&$createMenu, $createUrl) {
            ...
            foreach ($items as $item) {
                ...
                if (isset($item['submenu'])) {
                    $html .= "\n<li class='drop-down'><a href='{$url}' class='dropdown-toggle' data-toggle='dropdown' role='button' aria-expanded='false' title='{$item['title']}'>{$item['text']} <span class='caret'></span></a>{$submenu}</li>\n";
                } else if (isset($item['isSub'])) {
                    $isInsubMenu = true;
            ...
            return array("\n<ul class='nav navbar-nav navbar-right'>$html</ul>\n", $hasItemIsSelected);
        };
